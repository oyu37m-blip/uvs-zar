...
export default function ListingPage() {
  ...
  if (loading) return <div style={{ padding: 40, textAlign: 'center' }}>Unshij baina...</div>
  if (!listing) return <div style={{ padding: 40, textAlign: 'center' }}>Zar oldsongui</div>
  ...
  return (
    <main style={{ maxWidth: 480, margin: '0 auto', fontFamily: 'sans-serif', background: '#f5f5f5', minHeight: '100vh' }}>
      <div style={{ background: '#fff', padding: '14px 16px', display: 'flex', alignItems: 'center', gap: 12, borderBottom: '1px solid #eee', position: 'sticky', top: 0, zIndex: 10 }}>
        ...
        <div style={{ fontSize: 17, fontWeight: 700 }}>Delgerenguui</div>
      </div>
      ...
      <div style={{ padding: 16, display: 'flex', flexDirection: 'column', gap: 12 }}>
        <div style={{ background: '#fff', borderRadius: 12, padding: 16 }}>
          <div style={{ fontSize: 24, fontWeight: 700, color: '#1D9E75' }}>
            {listing.price ? '₮' + listing.price.toLocaleString() : 'Une tohoroltsono'}
          </div>
        ...
        {listing.description && (
          <div style={{ background: '#fff', borderRadius: 12, padding: 16 }}>
            <div style={{ fontSize: 14, fontWeight: 600, marginBottom: 8 }}>Tailbar</div>
            <div style={{ fontSize: 14, color: '#555', lineHeight: 1.6 }}>{listing.description}</div>
          </div>
        )}
        <div style={{ background: '#fff', borderRadius: 12, padding: 16 }}>
          <div style={{ fontSize: 14, fontWeight: 600, marginBottom: 8 }}>Holboo barih</div>
